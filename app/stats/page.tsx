...
import { getStats, type GetStats200ConsistencyByDay } from "@/app/_lib/api/fetch-generated";
...
// getStats' workoutStreak field is unreliable (returns 0 even when
// consistencyByDay confirms today was completed), so we derive it
// ourselves from consistencyByDay instead of trusting the API field.
function computeWorkoutStreak(
  ...
  while (consistencyByDay[cursor.format("YYYY-MM-DD")]?.workoutDayCompleted) {
    streak += 1;
...
export default async function StatsPage() {
  ...
  const workoutStreak = computeWorkoutStreak(consistencyByDay, today);
